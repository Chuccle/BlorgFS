...
int main()
{
    ...
    // 1b. Sanity check on BlorgTlsSha256 over real (non-empty) input, independent
    ...
    // SHA-256(ClientHello || ServerHello) directly.
    {
        ...
        CheckStatus("BlorgTlsSha256(ClientHello||ServerHello)", BlorgTlsSha256(buf, sizeof(buf), hash));
    ...
    CheckStatus("BlorgTlsHkdfExpandLabel(derived, from early)",
    ...
    CheckStatus("BlorgTlsHkdfExtract(handshake)",
        BlorgTlsHkdfExtract(derivedForHandshake, TLS_HASH_LEN, Rfc8448.SharedSecretIkm, TLS_HASH_LEN, handshakeSecret));
    ...
    CheckStatus("BlorgTlsHkdfExpandLabel(c hs traffic)",
        BlorgTlsHkdfExpandLabel(handshakeSecret, TLS_HASH_LEN, "c hs traffic", Rfc8448.TranscriptHashChSh, TLS_HASH_LEN, TLS_HASH_LEN, clientHsTraffic));
    ...
    CheckStatus("BlorgTlsHkdfExpandLabel(s hs traffic)",
        BlorgTlsHkdfExpandLabel(handshakeSecret, TLS_HASH_LEN, "s hs traffic", Rfc8448.TranscriptHashChSh, TLS_HASH_LEN, TLS_HASH_LEN, serverHsTraffic));
    ...
    CheckStatus("BlorgTlsHkdfExpandLabel(derived, from handshake)",
        BlorgTlsHkdfExpandLabel(handshakeSecret, TLS_HASH_LEN, "derived", emptyHash, TLS_HASH_LEN, TLS_HASH_LEN, derivedForMaster));
    ...
    CheckStatus("BlorgTlsHkdfExtract(master)",
        BlorgTlsHkdfExtract(derivedForMaster, TLS_HASH_LEN, zero32, 32, masterSecret));
    ...
    CheckStatus("BlorgTlsHkdfExpandLabel(key)",
        BlorgTlsHkdfExpandLabel(serverHsTraffic, TLS_HASH_LEN, "key", NULL, 0, TLS_KEY_LEN, serverKey));
    CompareBytes("server_hs_write_key", serverKey, Rfc8448.ServerHsWriteKey, TLS_KEY_LEN);

    unsigned char serverIv[TLS_IV_LEN];
    CheckStatus("BlorgTlsHkdfExpandLabel(iv)",
        BlorgTlsHkdfExpandLabel(serverHsTraffic, TLS_HASH_LEN, "iv", NULL, 0, TLS_IV_LEN, serverIv));
    ...
    CheckStatus("BlorgTlsHkdfExpandLabel(finished)",
        BlorgTlsHkdfExpandLabel(serverHsTraffic, TLS_HASH_LEN, "finished", NULL, 0, TLS_HASH_LEN, finishedKey));
    ...
    unsigned char transcriptThroughCertVerify[TLS_HASH_LEN];
    {
        ...
        CheckStatus("BlorgTlsSha256(transcript through CertificateVerify)",
            BlorgTlsSha256(buf, sizeof(buf), transcriptThroughCertVerify));
    }

    unsigned char finishedMac[TLS_HASH_LEN];
    CheckStatus("BlorgTlsHmacSha256(finished)",
        BlorgTlsHmacSha256(finishedKey, TLS_HASH_LEN, transcriptThroughCertVerify, TLS_HASH_LEN, finishedMac));
    ...
    CheckStatus("BlorgTlsAeadEncrypt(server record 0)",
        BlorgTlsAeadEncrypt(Rfc8448.ServerHsWriteKey, Rfc8448.ServerHsWriteIv, 0, aad, 5, plaintext, 658, ciphertext, tag));
    ...
    CheckStatus("BlorgTlsAeadDecrypt(server record 0)",
        BlorgTlsAeadDecrypt(Rfc8448.ServerHsWriteKey, Rfc8448.ServerHsWriteIv, 0, aad, 5,
    ...
    badTag[0] ^= 0xFF;
    NTSTATUS tamperStatus = BlorgTlsAeadDecrypt(Rfc8448.ServerHsWriteKey, Rfc8448.ServerHsWriteIv, 0, aad, 5,
    ...
    // BCRYPT_KDF_RAW_SECRET byte-reversal in BlorgTlsEcdhComputeSharedSecret
    // is the right way round, rather than an assumption.
    unsigned char sharedFromClient[TLS_ECC_COORD_LEN];
    CheckStatus("BlorgTlsEcdhComputeSharedSecret(client side)",
        BlorgTlsEcdhComputeSharedSecret(Rfc8448.P256ClientPrivate, Rfc8448.P256ClientPublic, Rfc8448.P256ServerPublic, sharedFromClient));
    ...
    CheckStatus("BlorgTlsEcdhComputeSharedSecret(server side)",
        BlorgTlsEcdhComputeSharedSecret(Rfc8448.P256ServerPrivate, Rfc8448.P256ServerPublic, Rfc8448.P256ClientPublic, sharedFromServer));
    ...
    // BlorgTlsEncodeP256SubjectPublicKeyInfo/BlorgTlsDecodeP256SubjectPublicKeyInfo,
    // and confirms BlorgTlsEcdsaVerify accepts the real signature and rejects
    ...
    // code parses a real external certificate.
    {
        ...
        CheckStatus("BlorgTlsSha256(test message)", BlorgTlsSha256(reinterpret_cast<const unsigned char*>("stage 2 test"), 12, testHash));
        ...
        CheckStatus("BlorgTlsEncodeP256SubjectPublicKeyInfo", BlorgTlsEncodeP256SubjectPublicKeyInfo(publicKey, der));

        unsigned char decodedPublicKey[TLS_ECC_PUBKEY_LEN];
        CheckStatus("BlorgTlsDecodeP256SubjectPublicKeyInfo", BlorgTlsDecodeP256SubjectPublicKeyInfo(der, TLS_SPKI_DER_LEN, decodedPublicKey));
        ...
        CheckStatus("BlorgTlsEcdsaVerify(real signature)", BlorgTlsEcdsaVerify(decodedPublicKey, testHash, TLS_HASH_LEN, signature));

        unsigned char tamperedSig[64];
        memcpy(tamperedSig, signature, 64);
        tamperedSig[0] ^= 0xFF;
        NTSTATUS verifyTamperedStatus = BlorgTlsEcdsaVerify(decodedPublicKey, testHash, TLS_HASH_LEN, tamperedSig);
        ...
        NTSTATUS decodeCorruptStatus = BlorgTlsDecodeP256SubjectPublicKeyInfo(corruptDer, TLS_SPKI_DER_LEN, decodedPublicKey);
