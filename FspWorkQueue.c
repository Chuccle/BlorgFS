...
IO_CSQ_INSERT_IRP FspCsqInsertIrp;
IO_CSQ_REMOVE_IRP FspCsqRemoveIrp;
IO_CSQ_PEEK_NEXT_IRP FspCsqPeekNextIrp;
IO_CSQ_ACQUIRE_LOCK FspCsqAcquireLock;
IO_CSQ_RELEASE_LOCK FspCsqReleaseLock;
IO_CSQ_COMPLETE_CANCELED_IRP FspCsqCompleteCanceledIrp;
...
VOID FspCsqAcquireLock(IO_CSQ* Csq, PKIRQL Irql)
{
    UNREFERENCED_PARAMETER(Csq);
    KeAcquireSpinLock(&g_FspOverflowQueueSpinLock, Irql);
}

VOID FspCsqReleaseLock(IO_CSQ* Csq, KIRQL Irql)
{
    UNREFERENCED_PARAMETER(Csq);
