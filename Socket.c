...
NTSTATUS CreateWskSocket(PKSOCKET* Socket, USHORT SocketType, ULONG Protocol, ULONG Flags, PSOCKADDR RemoteAddress)
{
    *Socket = NULL;
    ...
    PKSOCKET pNewSocket = ExAllocatePoolZero(NonPagedPoolNx, sizeof(KSOCKET), SOCKET_TAG);
    ...
    NTSTATUS result = InitialiseSocketContext(&pNewSocket->SocketContext);
    ...
    result = WskProviderNpi.Dispatch->WskSocketConnect(
        ...
        NULL,
        NULL,
        pNewSocket->SocketContext.Irp
    );

    WaitForCompletionSocketContext(&pNewSocket->SocketContext, &result);

    if (NT_SUCCESS(result))
    {
        pNewSocket->WskSocket = (PWSK_SOCKET)pNewSocket->SocketContext.Irp->IoStatus.Information;
